...
export function LootTableEntry<T extends string = string>(id: T | null, weight: number = 1, min: number = 1, max: number = 1, step: number = 1, group: number = 1): ILootTableEntry {
...
export function CheckLootTableEntry<T extends string = string>(entry: AtLeast<ILootTableEntry<T, string | number>, "id">): AtLeast<ILootTableEntry<T, string | number>, "id"> {
    if (entry.id !== null && !rxLootTableEntryID.test(entry.id)) throw Error(`LootTableEntry ${entry.id} invalid id format.`);
    if (typeof entry.min === "number" && typeof entry.max === "number" && entry.min > entry.max) throw Error(`LootTableEntry ${entry.id} min must be less than or equal to max.`);
    if (typeof entry.step === "number" && (!isPositiveInt(entry.step) || entry.step == 0) && !Number.isNaN(entry.step)) throw Error(`LootTableEntry ${entry.id} step must be a positive integer or NaN.`);
